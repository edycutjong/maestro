## [0.3.2](https://github.com/edycutjong/maestro/compare/v0.3.1...v0.3.2) (2026-07-11)


### Bug Fixes

* **deps:** bump croo-core 0.4.0 -> 0.5.1 (fixes sub-agent hire timeout) ([4e75330](https://github.com/edycutjong/maestro/commit/4e75330e7bc0a1b2e9e61d2ae5735937b21f2a67))

## [0.3.1](https://github.com/edycutjong/maestro/compare/v0.3.0...v0.3.1) (2026-07-08)


### Bug Fixes

* **icon:** animated icon renders on GitHub (SMIL + real transform) ([5740368](https://github.com/edycutjong/maestro/commit/57403686d94e2a5cad134c5c8fb672a36c97692d))

# [0.3.0](https://github.com/edycutjong/maestro/compare/v0.2.2...v0.3.0) (2026-07-08)


### Features

* **icon:** waypoints glyph + Agent Store badge, hire example, community files ([c66e773](https://github.com/edycutjong/maestro/commit/c66e77334aed67ffc2a8b9238206d578d228d4fd))

## [0.2.2](https://github.com/edycutjong/maestro/compare/v0.2.1...v0.2.2) (2026-07-07)


### Bug Fixes

* accept free-text requirement + parse sub-agent JSON deliverables ([bb0ed60](https://github.com/edycutjong/maestro/commit/bb0ed600f17c991b00db1456ab42095471c042e7))

## [0.2.1](https://github.com/edycutjong/maestro/compare/v0.2.0...v0.2.1) (2026-07-06)


### Bug Fixes

* **deps:** resolve @edycutjong/croo-core from npm registry (was broken file: link, broke CI) ([5f9a69a](https://github.com/edycutjong/maestro/commit/5f9a69a1734dd9e831e7e9ad73e2d4f48a73d6cb))

# [0.2.0](https://github.com/edycutjong/maestro/compare/v0.1.0...v0.2.0) (2026-07-06)


### Features

* add reject/expiry hire cascade and HTTP trace-UI server ([476787b](https://github.com/edycutjong/maestro/commit/476787b55fc5717b3762274e5f113eb2a4510c3f))
